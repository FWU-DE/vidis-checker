...
Hebe die einzelnen Punkte einzeln hervor (mit Hilfe der Textformatierung) und zeige an, ob der Punkt erfüllt (✅), nicht erfüllt (❌) oder möglicherweise zu prüfen ist (⚠️).
Im nachfolgenden Text findest du die Datenschutzerklärung.

Text:
$PRIVACY_POLICY

Abschließend gib in einem letzten Satz an, ob die Datenschutzerklärung insgesamt vollständig ist.
...
privacy_policy_check_prompt_dpo_only = """
Prüfe die Nennung des Datenschutzbeauftragten in der nachfolgenden Datenschutzerklärung.

Text:
$PRIVACY_POLICY

Abschließend gib in einem letzten Satz an, ob der Datenschutzbeauftragte genannt ist.
Schreibe nur 'True' wenn der Datenschutzbeauftragte genannt ist, ansonsten 'False'.
"""


def get_privacy_policy_check_prompt(privacy_policy_text: str, dpo_only: bool = False) -> str:
    if dpo_only:
        return privacy_policy_check_prompt_dpo_only.replace("$PRIVACY_POLICY", privacy_policy_text)
    else:
        return privacy_policy_check_prompt.replace("$PRIVACY_POLICY", privacy_policy_text)


def check_privacy_policy_from_text(privacy_policy_text: str, dpo_only: bool = False) -> tuple[bool, str]:
    prompt = get_privacy_policy_check_prompt(privacy_policy_text, dpo_only)
    response = generate_text(prompt)

    # Extract the last sentence to determine if the privacy policy is complete
...
def check_privacy_policy(file_path: str, dpo_only: bool = False) -> tuple[bool, str]:
    privacy_policy_text = read_text_from_pdf(file_path)
    return check_privacy_policy_from_text(privacy_policy_text, dpo_only)


if __name__ == "__main__":
    result, explanation = check_privacy_policy(
        "tmp/schooltogo.de_legal/privacy_policy.pdf", dpo_only=False
    )
    print(f"Datenschutzerklärung vollständig: {result}")
    print(explanation)
